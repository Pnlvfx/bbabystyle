...
export async function GET() {
  const communities = await ssrapis.getSitemap('community') as CommunityProps[]
  if (!communities) return new Response(`<div >Something went wrong!</div>`, {
    headers: {
      'Content-Type': 'text/html'
    }
  })
  const content = communities.map((c) => `<url><loc>${process.env.NEXT_PUBLIC_CLIENT_URL}/b/${c.name.toLowerCase()}</loc></url>`).join('')
