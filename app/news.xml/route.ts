...
export async function GET() {
  const news = await ssrapis.getSitemap('news')
  if (!news) {
    return new Response('<div>Something went wrong</div', { headers: { 'conten-type': 'text/xml' } })
  }
  const content = (news as NewsProps[]).map((news) => `<url><loc>${process.env.NEXT_PUBLIC_CLIENT_URL}${news.permalink}</loc></url>`).join('')
  return new Response(
