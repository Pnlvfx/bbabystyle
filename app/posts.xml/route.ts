...
export async function GET() {
  const posts = await ssrapis.getSitemap('post')
  if (!posts) {
    return new Response('<div>Something went wrong</div', { headers: { 'conten-type': 'text/xml' } })
  }
  const content = (posts as PostProps[]).map((post) => `<url><loc>${process.env.NEXT_PUBLIC_CLIENT_URL}${post.permalink}</loc></url>`).join('')
  return new Response(
