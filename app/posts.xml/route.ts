...
export async function GET() {
  const posts = await ssrapis.getSitemap('post') as PostProps[]
  if (!posts) return new Response(`<div >Something went wrong!</div>`, {
    headers: {
      'Content-Type': 'text/html'
    }
  })
  const content = posts.map((post) => `<url><loc>${process.env.NEXT_PUBLIC_CLIENT_URL}${post.permalink}</loc></url>`).join('')
