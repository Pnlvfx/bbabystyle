...
const TweetHeader = ({ user_avatar, username, screen_name, created_at, userId, isMobile }: TweetHeaderProps) => {
  return (
    <div className="relative mx-2 mb-2 flex items-start text-[12px] leading-4">
      <div className="flex-none align-baseline">
        <Link
          ...
          className={`inline align-baseline font-bold leading-5 ${isMobile && 'articleLink'}`}
        >
      ...
      <div className="flex shrink grow flex-wrap items-center overflow-hidden">
        <div className="inline items-center leading-4">
          <div className="inline-block flex-none">
            <Link
              href={`/governance/twitter/user/${userId}`}
              className={`inline align-baseline font-bold leading-5 hover:underline ${isMobile && 'articleLink'}`}
            >
              {username}
            </Link>
          </div>
          ...
          <div className=" inline-block flex-none text-reddit_text-darker">
            <div>
              <Link href={`/governance/twitter/user/${userId}`} className={`hover:underline ${isMobile && 'articleLink'}`}>
                @{screen_name}
              </Link>
            </div>
          </div>
